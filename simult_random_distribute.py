...
#Usage:
#    $ ./simult_random_distribute.py [simulate date file] [request detail record file]
#Date:
#    2014.08.04
...
for second in range(7200):
    ...
    for i in range(5):
       ...
       # print 'num:%d,%d,%d'%(num[0],num[1],num[2])
        ...
        shell="solve_distribute/solve %f %f %f %d %d %d %f" % (qloadlen[0],qloadlen[1],qloadlen[2],num[0],num[1],num[2],ra[i])
    ...
    while(req_next.request_time==second):
        ...
        two = sample(range(0,5),2)
        id0 = two[0]
        id1 = two[1]
        if theirlen[id0]>theirlen[id1]:
            sid = id1
        else:
            sid = id0
